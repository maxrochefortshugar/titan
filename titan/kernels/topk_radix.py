...
"""Fused top-K over one very wide logits row.

Ported from ``engine/patches/round3/small-items/topk.py``.

The MTP shortlist drafter selects K candidate rows out of a 248,320-wide
lm_head row once per decode cycle. ``mx.topk`` / ``mx.argpartition`` cost 0.27
to 0.38 ms there against 0.027 ms for ``mx.argmax`` over the same array,
because the partition sorts far more than it has to.

This is a three-launch radix select over the order-preserving uint32 key of
each float:

1. ``hist``   2048-bin histogram of the top 11 key bits, threadgroup-private
   histograms combined with one device atomic per non-empty bin.
2. ``split``  every threadgroup redundantly suffix-scans that histogram to find
   the boundary bucket and the count strictly above it, emits those winners
   directly, and compacts the boundary bucket into a candidate list.
3. ``refine`` one threadgroup selects the remaining winners out of the
   candidate list exactly, with an 11-bit then a 10-bit in-threadgroup
   histogram, so all 32 key bits are resolved.

Exactness: a correct top-K for any input, no tolerance and no fallback. Ties at
the boundary value are broken arbitrarily, exactly as ``mx.topk`` breaks them
arbitrarily, so the two agree as sets whenever the K-th largest value is unique
and always agree as multisets of values.
"""
...
def _kernels():
    ...
    if k is None:
        k = (
            mx.fast.metal_kernel(name="titan_topk_hist", input_names=["x"],
                                 output_names=["hist"], header=_HEADER,
                                 source=_SRC_HIST),
            mx.fast.metal_kernel(name="titan_topk_split", input_names=["x", "hist"],
                                 output_names=["oidx", "oval", "cand", "ctr"],
                                 header=_HEADER, source=_SRC_SPLIT),
            mx.fast.metal_kernel(
                name="titan_topk_refine",
                input_names=["x", "cand", "ctr", "iidx", "ival"],
                output_names=["oidx", "oval"], header=_HEADER, source=_SRC_REFINE),
        )
        _CACHE["k"] = k
    return k
...
def reference(x, k):
    """Top-k of one row through MLX. Returns ``(values, indices)``, ``[k]`` each,
    in no particular order, indices uint32."""
    row = x.reshape(-1)
    v = int(row.size)
    k = max(1, min(int(k), v))
    idx = mx.argpartition(row, kth=v - k, axis=-1)[-k:].astype(mx.uint32)
    return row[idx], idx


def metal(x, k):
    """Three-launch radix select. Same signature as :func:`reference`."""
    row = x.reshape(-1)
    v = int(row.size)
    k = max(1, min(int(k), v))
    kh, ks, kr = _kernels()
    ...
    (hist,) = kh(
        ...
        template=[("T", row.dtype), ("N", v), ("NTHREADS", nthreads),
                  ("NBINS", _NBINS)],
        grid=(nthreads, 1, 1), threadgroup=(_TG, 1, 1),
        output_shapes=[(_NBINS,)], output_dtypes=[mx.uint32], init_value=0,
    )
    oidx, oval, cand, ctr = ks(
        ...
        output_dtypes=[mx.uint32, row.dtype, mx.uint32, mx.uint32], init_value=0,
    )
    fidx, fval = kr(
        ...
        output_shapes=[(k,), (k,)],
        output_dtypes=[mx.uint32, row.dtype], init_value=0,
    )
    return fval, fidx


def key(x, k) -> ShapeClass:
    return shape_class(x, extra=(int(k),))


def supports(sc: ShapeClass) -> bool:
    if sc.device != "gpu" or len(sc.shapes) != 1 or not sc.extra:
        return False
    shape = sc.shapes[0]
    ...
    n = shape[-1]
    k = sc.extra[0]
    if sc.dtypes[0] not in (mx.float32, mx.float16, mx.bfloat16):
        return False
    # the refine pass runs in one threadgroup, so K must fit its write budget
    return 1 <= k <= 8192 and n >= k


OP = KernelOp(
    name="topk_radix",
    aliases=("mtp_shortlist",),
    ...
    exactness="exact top-K set; ties broken arbitrarily as mx.topk does",
    source="engine/patches/round3/small-items/REPORT.md item 1",
)
