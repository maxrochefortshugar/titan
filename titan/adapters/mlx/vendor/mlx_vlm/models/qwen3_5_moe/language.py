...
def _target_verify_switch_glu(switch_mlp: SwitchGLU, x, indices, target_verify: bool):
    ...
    # Titan modification: same fused [gate | up] layout as SwitchGLU.__call__.
    if "gate_up_proj" in switch_mlp:
        gate, up = mx.split(
            switch_mlp.gate_up_proj(flat_x, flat_indices, sorted_indices=False),
            2,
            axis=-1,
        )
    else:
...
class Qwen3_5MoeSparseMoeBlock(nn.Module):
    ...
    def __call__(
        ...
        # Titan modification: ``moe.weighted_sum`` (round2/wsum10, extended for
        # the target-verify layout by round3/small-items) replaces the routed
        # weighted-sum tail with one launch. It is bit-identical to the ops
        # below in its default clone mode, and takes both layouts: the expert
        # output is token-major here, so the unsort permutation is None.
        y = _target_verify_switch_glu(self.switch_mlp, x, inds, target_verify)
        weighted_sum = _titan_op("moe.weighted_sum")
        if weighted_sum is not None:
            y = weighted_sum(y, None, scores, (*y.shape[:-2], y.shape[-1]))
        else:
