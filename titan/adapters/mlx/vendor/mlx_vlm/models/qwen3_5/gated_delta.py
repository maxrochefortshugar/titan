...
def gated_delta_update(
    ...
    # Titan modification: ``gdn.chunk_scan`` is the C=8 chunked delta-rule scan
    # (round3/gdn-scan, ported from mlx PR #4020).  Like the stock bf16 kernel
    # it is approximate -- y matches stock to four digits, state rrmse 6.1e-7 --
    # and it only takes prefill-shaped calls with scalar gating and no mask.
    if mask is None and q.shape[1] > 1 and g.ndim == 3:
        chunk_scan = _titan_op("gdn.chunk_scan")
        if chunk_scan is not None:
            result = chunk_scan(q, k, v, g, beta, state)
