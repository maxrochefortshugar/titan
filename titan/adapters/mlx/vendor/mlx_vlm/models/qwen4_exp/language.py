...
# Titan modification: the PLE table is always served from the checkpoint on
# SSD ("mmap"); the resident 32 GB variant is gone (see VENDORED.md).  The
# model path arrives on TextConfig, not in a module global.
PLE_RUNTIME_MODE = "mmap"
...
def _gathered_min_query_tokens() -> int:
    ...
    raw = os.environ.get("TITAN_QWEN4_GATHERED_MIN_QUERY", "").strip()
    if raw:
        try:
...
def mtp_runtime(config: TextConfig) -> Qwen4ExpMTPRuntime:
    """The Lightning MTP decision for *config*.

    Titan modification: the stock module kept the decision in a process global
    written by ``configure_mtp_runtime`` before each load.  Titan resolves it
    from the config the loader built, so two models can coexist in one process.
    """
    prefix = config.titan_mtp_checkpoint_prefix
    if config.titan_mtp_enabled and prefix is None:
        from ....checkpoint import checkpoint_mtp_weight_prefix

        # Titan modification: was
        #   from omlx.utils.model_loading import _checkpoint_qwen4_mtp_weight_prefix
        prefix = checkpoint_mtp_weight_prefix(config.titan_model_path or "")
    return Qwen4ExpMTPRuntime(
        enabled=bool(config.titan_mtp_enabled and prefix),
        checkpoint_prefix=prefix,
    )


def get_ple_runtime_mode() -> str:
    """Always ``"mmap"``.  Titan does not build the resident PLE table."""
    return PLE_RUNTIME_MODE
...
class Qwen4ExpRMSNormGated(nn.Module):
    ...
    def __call__(self, x: mx.array, gate: mx.array) -> mx.array:
        # Titan modification: ``gdn.norm_gate_fused`` (round2/gdn-norm, bit
        # identical to the body below) folds the grouped RMS norm and the
        # output gate into one launch for T > 1.  T == 1 and any registry miss
        # keep the stock ops.
        if (
            x.ndim == 4
            ...
            fused = _titan_op("gdn.norm_gate_fused")
            if fused is not None:
                out = fused(
                    ...
                    eps=self.eps,
                    activation=0 if self.activation == "sigmoid" else 1,
                )
                if out is not None:
                    return out
        dtype = x.dtype
...
class Qwen4ExpAttention(Qwen3_5Attention):
    ...
    def __call__(
        ...
        if self._gathered_text_prefill_eligible(
            ...
            cache._titan_last_prefill_gathered = True
...
class Qwen4ExpGatedResidual(nn.Module):
    ...
    def _forward(self, hyper_input: mx.array, target_verify: bool = False):
        ...
        hybrid = None
        if (
            getattr(self, "_titan_exact_hybrid_projection", False)
            and not target_verify
...
def _can_fuse_hyper_connection(module: Qwen4ExpGatedResidual) -> bool:
    ...
    if (
        ...
        or getattr(module, "_titan_exact_hybrid_projection", False)
    ):
        return False
...
class DiskBackedShardedEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self._prefix = prefix
        # Titan modification: the packed-rows reader is opened per checkpoint,
        # so the embedding remembers where it came from.
    ...
    def prefetch(self, indices: mx.array) -> None:
        ...
        if host.size == 0:
            return
        # Titan modification: hand the gather-ahead to the packed reader when
        # it is registered; it owns its own IO pool.
        packed_prefetch = _titan_op("ple.packed_rows_prefetch")
        table = (
            _titan_ple_table(self._prefix, self._model_path)
            if packed_prefetch
            else None
        )
        if table is not None:
            packed_prefetch(table, host)
            return
    ...
    def __call__(self, indices: mx.array) -> mx.array:
        ...
        # Titan modification: ``ple.packed_rows_lookup`` reads the packed
        # contiguous table (one pread per row instead of three), rows layout
        # only -- there is no resident mode in this tree.  It is bit-exact
        # against the shards below, which stay as the fallback when no packed
        # table has been built.
        packed = _titan_op("ple.packed_rows_lookup")
        table = _titan_ple_table(self._prefix, self._model_path) if packed else None
        if table is not None:
            values = packed(table, host)
...
class Qwen4ExpNGramEmbedding(nn.Module):
    def __init__(
        ...
        # Titan modification: rows mode only.  The table is always read from
        # the checkpoint on SSD; the resident variant no longer exists, and the
        # model path comes from the config rather than a module global.
        if config.titan_model_path is None:
...
class Qwen4ExpModel(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Titan modification: the stock body called into
        # omlx.patches.mlx_lm_mtp.prompt_priming here to stash the last prefill
        # hidden state on the generation host.  Titan's ModelState owns the MTP
        # hidden explicitly (see titan/adapters/mlx/state.py), so the hook is
        # gone; prefill() reads the hidden from the return value instead.
...
class LanguageModel(Qwen3_5LanguageModel):
    ...
    def bind_mtp_owner(self, owner, depth: int = 1) -> None:
        """Reference a root-level Lightning MTP head without double registration."""
        self._titan_qwen4_mtp_owner = weakref.ref(owner)
        self._enable_mtp_decode_markers(depth)

    def _enable_mtp_decode_markers(self, depth: int = 1) -> None:
        # Titan modification: the depth came from omlx.patches.mlx_lm_mtp's
        # process-global env setting.  Titan passes it in from ModelConfigs so
        # the module holds no global mutable state.
        self._titan_mtp_decode_enabled = True
        self._titan_mtp_chain = True
        self._titan_mtp_depth = int(depth)
        self._titan_mtp_head_prenorm = True

    def get_mtp_module(self):
        module = getattr(self, "mtp", None)
        if module is not None:
            return module
        owner_ref = getattr(self, "_titan_qwen4_mtp_owner", None)
