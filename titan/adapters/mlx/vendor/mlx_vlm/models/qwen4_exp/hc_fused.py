...
external evaluation propagate to the caller. Disable with TITAN_QWEN4_HC_FUSED=0.
"""
...
from .....kernels import hc_weights as _titan_hc_weights
...
_DISABLED = os.environ.get("TITAN_QWEN4_HC_FUSED", "1").strip().lower() in {
    "0",
    "false",
    "no",
    "off",
...
def _eps_array(module) -> mx.array:
    eps = getattr(module, "_titan_hc_fused_eps", None)
    if eps is None:
        eps = mx.array([float(module.hc_norm.eps)], dtype=mx.float32)
        mx.eval(eps)
        module._titan_hc_fused_eps = eps
    return eps
...
def _kernel_norm(module, flat, rows, hc, hidden, dtype):
    width = hc * hidden
    return _kernel(
        "titan_qwen4_hc_fused_norm", ["x", "w", "eps"], ["xn"], _N_SOURCE
    )(
...
def prefill_forward(module, hyper_input):
    ...
    Titan modification: two op-registry hooks.  ``hc.prefill_block_fused``
    replaces this whole body with the six-launch fused block (round3/hc-fuse,
    bit-identical to the code below); failing that, ``norm.grouped_rms_bf16``
    replaces the grouped RMS norm alone (ple-fix/norm_patch, within one bf16
    ULP).  Both fail closed to the canonical MLX path.
    """
    global _RUNTIME_FAILED, _FAILURE_LOGGED
    fused_block = _titan_op("hc.prefill_block_fused")
    if fused_block is not None:
        weights = _titan_hc_weights(module)
        if weights is not None:
            try:
                return fused_block(hyper_input, weights)
            except Exception as exc:  # noqa: BLE001 - fail closed to MLX
                logger.warning(
                    "fused hyper-connection block failed closed: %s", exc
                )
    try:
        hc, hidden = module.hc_count, module.hidden_size
        dtype = hyper_input.dtype
        grouped_norm = _titan_op("norm.grouped_rms_bf16")
        if grouped_norm is None:
            normed = module.hc_norm(hyper_input)
        else:
            rows = hyper_input.shape[0] * hyper_input.shape[1]
            normed = grouped_norm(
                hyper_input.reshape(rows, hc * hidden),
