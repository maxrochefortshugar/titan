...
def _nax_gpu() -> bool:
    """Whether the sparse-QSA ops are present at all.

    Titan modification: this used to probe ``omlx.custom_kernels.nax``.  The
    thresholds below exist because the MLX ops beat the native kernels at small
    row counts, so the question the callers are really asking is "is a native
    QSA kernel available", which is now a registry lookup.
    """
    return _titan_op("qsa.sparse_gqa") is not None
...
def _native_score_min_rows() -> int:
    ...
    return _min_rows("TITAN_QWEN4_QSA_NATIVE_SCORE_MIN_ROWS", 32)
...
def _native_topk_min_rows() -> int:
    ...
    return _min_rows("TITAN_QWEN4_QSA_NATIVE_TOPK_MIN_ROWS", 8)
...
def _native_main_min_rows() -> int:
    ...
    return _min_rows("TITAN_QWEN4_QSA_NATIVE_MAIN_MIN_ROWS", 24)
...
def _native_indexer_scores(
    ...
    *,
    ...
    try:
        op = _titan_op("qsa.indexer_scores")
        if op is None:
            _NATIVE_QSA_SCORE_DISABLED = True
            return None
        # The caller's [B,M,H,D] view transposes back to the GEMM-friendly
        # [B,H,M,D] ABI. The kernel only copies when the resulting view is not
        # row-contiguous (for example an offset query chunk).
        scores = op(
...
def _native_topk_indices(scores: mx.array, topk: int) -> mx.array | None:
    ...
    if scores.shape[1] < _native_topk_min_rows():
        return None
    try:
        op = _titan_op("qsa.topk_indices")
        if op is None:
            _NATIVE_QSA_TOPK_DISABLED = True
            return None
        indices = op(scores, topk=topk).astype(mx.int32)
...
def _decode_qsa_sdpa(
    ...
    try:
        op = _titan_op("qsa.decode_sdpa")
        if op is not None:
            out = op(queries, keys, values, scale, causal=False)
            if out is not None:
                return out
    except Exception:
    ...
    *,
    ...
    if query_chunk is None:
        ...
        if (
            ...
            if _titan_op("qsa.sparse_gqa") is not None:
