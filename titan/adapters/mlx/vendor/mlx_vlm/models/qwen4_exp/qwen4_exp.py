"""Vendored from mlx-vlm 0.6.3 (MIT), oMLX's ``qwen4_exp`` compat copy.

Titan modifications:
  * ``dequantize_fp8_weights`` now comes from the vendored Apache-2.0 copy in
    ``titan/adapters/mlx/vendor/fp8.py`` instead of ``omlx.patches``.
  * ``Model`` no longer builds a Qwen3-VL vision tower and no longer derives
    from the Qwen3.5 VLM ``Model``: it is a plain ``nn.Module`` holding the
    language model and the optional Lightning MTP head.  Titan is text-only.
  * The MTP runtime and PLE mode are passed in rather than read from oMLX's
    process-global env configuration.
"""
...
class Model(nn.Module):
    """Text-only Qwen3.8-Flash-Next root module (Titan modification).

    The stock class subclassed the Qwen3.5 VLM ``Model`` and constructed a
    Qwen3-VL vision tower.  Titan drops both: ``language_model`` and the
    optional ``mtp`` head are the whole model.
    """

    def __init__(self, config: ModelConfig):
        nn.Module.__init__(self)
        self.config = config
        self.mtp_runtime = mtp_runtime(config.text_config)
        self.language_model = LanguageModel(config.text_config, config)
        if self.mtp_runtime.enabled:
            self.mtp = Qwen4ExpMTPModule(config.text_config)
            self.language_model.bind_mtp_owner(
                self, config.text_config.titan_mtp_depth
            )

    def sanitize(self, weights):
        if get_ple_runtime_mode() == "mmap" and not getattr(
            self, "_titan_preserve_qwen4_ple_for_quantization", False
        ):  # always true in Titan: rows mode only
